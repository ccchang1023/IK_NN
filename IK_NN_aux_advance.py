...
class IK_NN(torch.nn.Module):
	def __init__(self, n_feature, n_neuron, n_output):
		...
		batchNorm_mometum = 0.1
		self.l1 = torch.nn.Linear(n_feature,n_neuron)
		self.b1 = torch.nn.BatchNorm1d(n_neuron,momentum=batchNorm_mometum)
		self.l2 = torch.nn.Linear(n_neuron,n_neuron)
		self.b2 = torch.nn.BatchNorm1d(n_neuron,momentum=batchNorm_mometum)
		self.l3 = torch.nn.Linear(n_neuron,n_neuron)
		self.b3 = torch.nn.BatchNorm1d(n_neuron,momentum=batchNorm_mometum)
		self.l4 = torch.nn.Linear(n_neuron,n_neuron)
		self.b4 = torch.nn.BatchNorm1d(n_neuron,momentum=batchNorm_mometum)
		self.out = torch.nn.Linear(n_neuron,n_output)
    
	def forward(self,x):
		x = F.tanh(self.b1(self.l1(x)))
		x = F.tanh(self.b2(self.l2(x)))
		x = F.tanh(self.b3(self.l3(x)))
		x = F.tanh(self.b4(self.l4(x)))
		x = self.out(x)
		return x


def load_model():
	model_save_path = "/home/ccchang/TD3/saved_models/aux_model_double"
	resume_model = "IK_model0.06254665470634756"
	model = IK_NN(n_feature=10, n_neuron=512, n_output=6)
	device = "cuda"
	if device == "cuda":
		###Load on GPU
		model = model.cuda()
	...
	for i in range(epochs):
		...
		if loss < min_loss:
			if i>5000:
				...
				torch.save(model.state_dict(),"./saved_models/aux_model_double/IK_model"+str(loss.item()))				
